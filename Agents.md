...
- 普通 insert/upsert/update/delete 使用按路径复制的 persistent row/index/receipt roots，并在一个脚本内合并原始 before 与最终 after；未修改表、row value 和 index posting 保持结构共享。Engine 用单个 committed root 原子发布 database 与 receipt；redb 从 write set 直接准备带 expected state 的 stable-key delta，普通提交后只长期保留小型 durable head。DDL、migration、upgrade、restore 与 receipt prune 继续明确走 full rebuild。
- `Engine::last_mutation_profile` 提供不含业务数据的 candidate/durable 分段耗时、增量/full-rebuild 模式与 row/receipt 写集计数；`tools/workload-eval` 保存对应原始样本和 p50/p95，并在接受写入样本前验证增量模式与写集形状。新的 10k/100k 容量结论留给 #166。
...
- 稳定分页契约见 `docs/rfc/0003-stable-cursor-pagination.md`：首版使用主键收尾的唯一 keyset 顺序和 sequence-pinned traversal；任意成功写入使旧 cursor 在扫描前明确过期。语言 `page`、Rust API 与 version 1 结构映射同一 PageSpec；HMAC cursor、bounded page 及完整 Rust/TCP/HTTP 旅程已由 #131/#132 实现。
- 六类生产标量已接通源码、Rust wrapper/serde、protocol v2、storage format 4、value/index/receipt codec、cursor、backup 与 migration。decimal 使用 `decimal P S` 和 context typed string literal，支持同类型 checked 加减/negation/sum；乘除、avg 与舍入保持 deferred。（历史：v0.10.0 已由 #247 补齐乘除、avg 与显式舍入，见下方发布候选条目。）
- 生产标量已接入 protocol v2 与完整 v1 typed-boundary 预检；新 redb 数据库使用 storage format 4 和 catalog/value/index-key/receipt codec 3/2/2/2，逻辑 backup 使用 codec 3。旧 format 1–3 保持可读且普通写入不隐式升到 4；`upgrade --db <path> --target 4` 在一个同步 two-phase transaction 中重写并验证全部 durable state。`.storage` 与 `check --db` 展示 codec versions；#137–#140 的中断、跨版本恢复、源码声明和运算验收均已完成。（历史：当前默认已随 v0.10.0 升为 storage format 10 与 codec 6/3/4/3。）
...
- v0.9 提供独立 `unionid-query` proc-macro crate：`queries! { schema "..." query name { ... } }` 在 Rust 编译期复用同一 parser、binder、query contract 与 codegen，生成共享 ADT 和 typed Params/Row/Output；consumer 验收覆盖 ADT match、mutation returning、memory/redb、schema drift 与四类 compile-fail。宏只读取声明式 schema，不打开 redb；migrated live catalog 继续使用 `query rust --db`。编译成本、重建与诊断边界见 `docs/benchmarks/query-macro-2026-09-18.md`。
- 部分唯一索引已按 RFC 0022 完成四个阶段：新库默认 format 10，journal 进入 11；catalog codec 6 保存 normalized predicate，logical backup 6 保存 predicate definition，显式 `upgrade --target 10/11` 支持 8→10 与 9→11。redb bounded `check` 按 predicate 验证 true 行恰有一个 posting、false 行没有 posting。planner 只从 barrier 前的简单 filter 与纯 `&&` 展平 atom 做 implication，`explain.plan.access` 输出 `index_predicate`、`predicate_proven` 和有界 `predicate_rejections`，稳定分页只在证明成立时用部分唯一索引论证唯一排序。`release_scenarios.rs` 的 `partial_email` 与 `active_external_id` 覆盖 query/DML → restart → migration → check → backup/restore，并有批量冲突与 migration 冲突的原子回滚回归；`src/portable.rs` 的机器描述已升为 version 2 并携带可选 canonical predicate，同时保留 version 1 读取。
- v0.10.0 发布候选：新库默认 storage format 10（catalog/value/index-key/receipt/maintenance/journal codec `6/3/4/3/1/0`），logical backup format 6，可读 storage format 1–11 与 backup format 1–6；`release/contract.json`、`docs/RELEASE-v0.10.0.md`、release workflow 的 v0.9.1 兼容性旅程以及 README/UPGRADING/RUST_QUERY_MACRO 已同步。
- `unionid agent --format json` 提供 version 1 机器可读能力清单：命令/用法、protocol/stream/storage 版本、错误契约字段（`code`/`message`/`span`/`constraint`/`hint`）、完整错误码词汇表、constraint 分类 hint、退出码分类与“读 schema → 生成 → 校验 → 执行”工作流；`tests/agent_manifest.rs` 覆盖 CLI 形态，`src/agent.rs` 的源码扫描测试保证错误码目录不漂移。
