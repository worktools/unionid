...
- `ConcurrentEngine::register_read` / `ReadOperation::start` / `cancel` 已实现 transport-neutral 可取消读取核心：active registry 64、terminal tombstone 256/60 秒，统一 `ExecutionControl` 在 admission、bind 与 query 循环按 cancel/shutdown/deadline 检查。operation capability 不持久化、不记录查询数据；wire stream 仍等待 #157。
