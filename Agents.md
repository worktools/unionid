...
- 生产标量已接入 protocol v2 与完整 v1 typed-boundary 预检；新 redb 数据库使用 storage format 4 和 catalog/value/index-key/receipt codec 3/2/2/2，逻辑 backup 使用 codec 3。旧 format 1–3 保持可读且普通写入不隐式升到 4；`upgrade --db <path> --target 4` 在一个同步 two-phase transaction 中重写并验证全部 durable state。`.storage` 与 `check --db` 展示 codec versions；#137–#140 的中断、跨版本恢复、源码声明和运算验收均已完成。
