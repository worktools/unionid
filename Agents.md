...
- 部分唯一索引的持久化边界已接通：新库默认 format 10，journal 进入 11；catalog codec 6 保存 normalized predicate，logical backup 6 保存 predicate definition，显式 `upgrade --target 10/11` 支持 8→10 与 9→11。redb bounded `check` 按 predicate 验证 true 行恰有一个 posting、false 行没有 posting；`tests/storage.rs`、`tests/migration.rs`、`tests/typed_maps.rs` 覆盖升级、migration、重启、check 与 backup/restore。planner 已按 RFC 0022 的保守规则做 predicate implication：只从 barrier 前的简单 filter 与纯 `&&` 展平 atom，逐项匹配 stable path/operator/类型/canonical value，`field == Some value` 可蕴含 `is_some`；`explain.plan.access` 输出 `index_predicate`、`predicate_proven` 和有界 `predicate_rejections`，稳定分页只在证明成立时用部分唯一索引论证唯一排序。软删除 email 与 Active external ID 的端到端业务场景仍待 #248 最后阶段。
