...
- 已实现命名 sum/record、tuple、option/list、严格插入、主键、filter/select、单键/多键 sort、前 N 行/范围 take，以及支持递归 pattern 与 option/sum/product/list 新值构造的 `filter match` 和 ADT derive；布尔/算术/函数表达式、更新操作及 migration 尚未实现。
