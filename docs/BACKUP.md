...
`checkpoint` 要求先 export 到数据库 head，再用新的已验证 baseline 提升可恢复下界。`prune` 默认只预览 manifest 已退休且不再被恢复点依赖的文件，只有带 `--confirm` 才删除同一有界选择。`disable` 默认拒绝丢弃尚未 export 的 journal；先 export，或在明确接受这些 sequence 不可恢复时同时传入 `--discard-unexported --confirm`。这些操作都不会把 format 7 原地降回 format 6。
...
Checkpoint requires export through the database head, then advances the recoverable floor with a new verified baseline. Prune is preview-only by default and selects only retired artifacts that no retained restore point needs; `--confirm` removes that same bounded selection. Disable rejects an unexported tail by default. Export first, or pass both `--discard-unexported` and `--confirm` only when abandoning those restore points is intentional. None of these operations downgrades format 7 in place.
