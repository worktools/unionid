...
unionid 的内置远程服务继续使用轻量 JSON Lines。需要 HTTP、认证、TLS、租户路由或应用 API 的生产服务，应把 version 1 `Request` / `Response` 放入自己的 HTTP 框架，并将请求交给 `server::execute_protocol_request_until`，同时传入该 HTTP 请求的绝对 deadline。数据库不会因此出现第二套 query parser 或执行语义。
...
- 多页 typed todo 遍历、真实客户端断开，以及断开后的健康请求；
...
- HTTP adapter 不应自行实现 key 表、digest 或 cursor；直接调用 `execute_protocol_request_until`，才能与 TCP/Rust 使用相同 canonical wire digest、原子 receipt、分页和错误契约。示例从请求进入 Engine 时计算五秒绝对 deadline。
...
- 只读 HTTP 服务应使用 `Engine::open_redb_read_only(path)`，或在已打开的 Engine 上调用 `with_read_only(true)`；所有 adapter 继续走 `execute_protocol_request` 或 `execute_protocol_request_until`，不要只在路由层按字符串猜测写语句。`introspection.read_only` 可作为启动检查。
- 当前 response 是有界完整 JSON；最多 1000 行的 keyset page 适合可恢复列表遍历。客户端断开不会成为可靠的取消信号，读取最多继续到 adapter deadline，并继续受工作行、排序内存和响应大小限制。显式取消与有背压的 NDJSON/streaming 等待 [#116](https://github.com/worktools/unionid/issues/116) 的一致性读快照，由 [#135](https://github.com/worktools/unionid/issues/135) 跟踪。
