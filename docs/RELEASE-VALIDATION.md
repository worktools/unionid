...
它们不模拟物理断电、文件系统违反同步承诺或设备损坏，也不提供延迟承诺。当前 1 万／10 万行的 query、write、check 与 shadow migration 数据见 [M7 验收记录](benchmarks/m7-acceptance-2026-09-10.md)；v0.8 双平台候选、v0.7 数据／backup／客户端兼容、原生包和空目录 starter 由 [#349](https://github.com/worktools/unionid/issues/349) 做最终验收。count_distinct、typed avg 与基础排名窗口另由语言和对应 feature tests 覆盖。
...
Run the suite with `cargo test --locked --test release_scenarios`. It covers application process boundaries and logical recovery, while physical power loss, broken filesystem synchronization, and hardware damage remain outside the test claim. The [M7 acceptance record](benchmarks/m7-acceptance-2026-09-10.md) retains current 10k/100k query, write, check, and shadow-migration evidence; [#349](https://github.com/worktools/unionid/issues/349) owns the final v0.8 dual-platform candidate, v0.7 data/backup/client compatibility, native archives, and empty-directory starter acceptance. Language and feature tests separately cover count_distinct, typed averages, and basic ranking windows.
