...
它们不模拟物理断电、文件系统违反同步承诺或设备损坏，也不提供延迟承诺。当前 1 万／10 万行的 query、write、check 与 shadow migration 数据见 [M7 验收记录](benchmarks/m7-acceptance-2026-09-10.md)；v0.9 双平台候选、v0.8 数据／backup／客户端兼容、原生包和空目录 starter 由 [#357](https://github.com/worktools/unionid/issues/357) 做最终验收。内联 Rust 查询宏由 macro crate tests 和 Linux release evaluator 覆盖。
