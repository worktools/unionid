...
它们不模拟物理断电、文件系统违反同步承诺或设备损坏，也不提供延迟承诺。当前 1 万／10 万行的 query、write、check 与 shadow migration 数据见 [M7 验收记录](benchmarks/m7-acceptance-2026-09-10.md)；v0.4 双平台候选、静态查询生成、独立 Rust consumer、SQLite + SQLx 配对 smoke 与包内教程由 [#269](https://github.com/worktools/unionid/issues/269) 做最终验收。
...
Run the suite with `cargo test --locked --test release_scenarios`. It covers application process boundaries and logical recovery, while physical power loss, broken filesystem synchronization, and hardware damage remain outside the test claim. The [M7 acceptance record](benchmarks/m7-acceptance-2026-09-10.md) retains current 10k/100k query, write, check, and shadow-migration evidence; [#269](https://github.com/worktools/unionid/issues/269) owns the final v0.4 dual-platform candidate, static-query generation, typed application evolution, paired SQLite + SQLx smoke, independent Rust consumer, and packaged-tutorial acceptance.
