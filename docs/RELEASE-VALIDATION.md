...
候选 job 还会按当前平台下载 GitHub 已发布的 v0.6.0 原生包并核对其公开 `.sha256`。`scripts/verify-previous-release-compatibility.py` 用 v0.6 binary 建库、检查并生成 logical backup，再要求 v0.7 保持 capability contract、typed rows 与 schema identity，恢复 v0.6 backup，并接受 v0.6 CLI 通过 version 1 TCP 查询 v0.7 read-only server。这条测试只覆盖已冻结的直接兼容路径，不把跨版本 mutation 或 format 降级加入承诺。
...
Each candidate job also downloads the published v0.6.0 native archive for its platform and verifies the public checksum. `scripts/verify-previous-release-compatibility.py` uses the v0.6 binary to create and check a database and logical backup, then requires v0.7 to preserve the capability contract, typed rows, and schema identity, restore the v0.6 backup, and accept a version-1 TCP query from the v0.6 CLI against a v0.7 read-only server. This proves the frozen direct path without promising cross-version mutations or storage-format downgrade.
