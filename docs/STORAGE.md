...
storage format 1 表示没有持久回执；新实现仍可直接打开。第一次成功提交 durable idempotency receipt 时，同一事务把 storage format 升为 2。format 2 不可用只认识 format 1 的旧二进制安全打开，因为忽略回执会让重试重复产生效果。memory Engine 使用同样的 key/digest/replay/容量规则，但回执只在进程内存在。完整语义与限制见 [RFC 0002](rfc/0002-idempotent-write-receipts.md)；version 1 网络字段和 canonical digest builder 由 #126 接入。
