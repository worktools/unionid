...
format-6 generation reclaim 删除旧 generation 的 catalog、row 和 index key，但 redb 已分配页面可能继续形成文件高水位。停止数据库 owner、保留 verified logical backup 并完成或 abort unfinished migration 后，可显式运行 `unionid compact --db app.redb`。它原地调用 redb native compaction，前后各运行完整物理／逻辑检查，并比较 schema、sequence、codec、stable ID/RowId 水位、ledger、receipt、generation、cursor identity 和 typed-row 摘要。成功不推进 sequence，不改变 cursor 或 receipt replay identity。

native compact 返回时文件长度小于持久化 region 布局，redb 会在下一次打开时 repair 并向上取整到 region 边界。实现因此在 compact 之后、post-check 之前重开数据库，使 repair 发生在同一次维护内；报告的 `after_bytes` 是重开后的 durable 大小，`changed` 只在文件实际变小时为 true。真实 generation reclaim 后该值显著下降（#229 churn 数据上 10k 约 45.98→21.87 MiB，100k 约 744.84→219.18 MiB）；再次运行同一文件稳定返回 `changed=false`、`reclaimed_bytes=0`。详见 [compact 空间收益与 no-op 证据](benchmarks/compaction-2026-09-11.md)。
...
Native compact returns while the file is shorter than redb's persisted region layout; the next open runs repair and rounds the file up to a region boundary. The implementation therefore reopens the database after compact and before the post-check so repair happens inside the same maintenance operation. The reported `after_bytes` is the durable post-reopen size and `changed` is true only when the file actually got smaller. On the #229 churn data this drops 10k from about 45.98 to 21.87 MiB and 100k from about 744.84 to 219.18 MiB, and a second run returns a stable `changed=false` with `reclaimed_bytes=0`. See the [compaction space-reduction and no-op evidence](benchmarks/compaction-2026-09-11.md). The operation is synchronous offline maintenance with full scans and multiple internal commits; after interruption or an uncertain result, reopen and run `check`.
...
| `backup_chain_state` | singleton key | versioned chain state | format 7 的 chain ID、baseline/export/head checksum、retained range 与容量 |
| `backup_journal` | `(sequence, ordinal)` | versioned canonical delta record | format 7 的 begin、catalog/row/ledger/receipt change 与 end checksum；不重复保存派生索引 |

format-5/6 普通打开会验证 meta、active catalog、schema hash、ledger 单链及其 head、receipt codec/容量边界，并确认对应 row/index 表存在；它不会遍历全部 row value 或 index key。format 6 还会验证 generation ID 单调关系、manifest codec，以及 active、source、target 和 unfinished state 不矛盾。显式启用 journal 的 format 7 进一步验证 state、连续 sequence/ordinal、canonical change 顺序、parent/head checksum、分类计数和容量。按需读取仍会拒绝触及的未知／损坏 value、index-key 或 generation-key codec。显式 `check` 才遍历 active generation 的全部 rows/indexes，验证 RowId 唯一性／顺序／分配水位和索引是否与 catalog/rows 一致。RowId 可以有删除形成的缺口。差异计划测试检查 update/delete/insert 只生成预期的 catalog/row/index 键变化；migration 测试确认 schema、数据、索引和 ledger 一起提交，普通数据提交保留 ledger。集成测试还会在一个未提交 redb transaction 修改多个内部表后直接退出子进程，确认重开只看到完整旧状态；也会在带 receipt 和 journal 的 Engine commit 成功后不执行析构直接退出，确认重开能重放完整新状态。无效 redb 文件会返回 `E_STORAGE` 并保留原文件，跨进程第二个打开者返回 `E_BUSY`。
