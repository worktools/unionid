...
- 原子 claim：按 id 和旧状态筛选，并用 `set state = match state` 把 `Queued` 改成 `Running`；最后的 `current => current` 保留其他状态。该写法已由 #83 接入现有 typed match IR，多个 set 仍同时读取修改前的 row。
...
| 原子状态转换、upsert、delete | update/delete 已实现 filter/match target、穷尽 ADT match assignment 与 typed simultaneous set；upsert 已实现按主键 insert/replace；三者维护约束、索引、affected rows、稳定 RowId 和 redb 增量键提交 | 扩大工作集时直接生成 mutation set | #15/#83 |
