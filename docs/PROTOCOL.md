...
version 2 增加 UUID、date、timestamp、duration、decimal 和 bytes wire value，canonical envelope 见 [RFC 0004](rfc/0004-production-scalars.md)。Rust request builder 默认仍为 version 1，可用 `.with_version(2)?` 选择 version 2；同一个 request_id 不影响幂等 identity，版本仍参与 canonical digest。当前源码语言已接入 UUID/bytes 类型和字面量；其余标量暂时只能经 Rust/wire 基础表示，等待各自实现任务。
