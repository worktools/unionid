...
当前已实现新参数解码、返回行、版本回显和完整 v1 typed-boundary 预检；v1 在执行 mutation 前检查嵌套参数、最终 query／`returning`／`explain` 结果类型及 introspection schema，并以 `E_PROTOCOL_TYPE` 拒绝无法表达的请求。幂等命中保持不解析源码的重放语义。新 redb 数据库使用 storage format 6；旧 format 1–3 必须显式升级到 4 才能持久写入生产标量，format 4 创建复合或降序索引前必须升级到 5，format 5 可继续读写或显式升级到 6。generation envelope 不改变 query protocol 或 typed value codec。Rust 与 codec 当前范围见 [SCALARS.md](SCALARS.md)。
