...
M7 已通过 [format-6 envelope #184](https://github.com/worktools/unionid/issues/184)、[可恢复维护 #185](https://github.com/worktools/unionid/issues/185) 和 [容量验收 #186](https://github.com/worktools/unionid/issues/186) 完成数据库核心边界。当前示例从空库使用 format 6，只验证应用数据分层方式，不构成宿主语言或网络层容量证明。
...
M7 #184/#185/#186 completed the generation envelope, resumable maintenance, and database capacity acceptance. This example starts from an empty format-6 database and validates only the application-data split; it does not establish host-runtime or network capacity.
