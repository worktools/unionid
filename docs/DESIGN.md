...
- record／tuple／sum／Option／List 提供结构相等与由静态类型驱动的 total order。sum 相等包括类型身份、变体身份和负载，排序先比较稳定 variant ID 再比较 payload；record 按稳定 field ID，Option 使用 `None < Some`，list 使用短前缀优先的词典序。该相等与顺序同时用于 filter、sort、count_distinct、min/max 与 cursor boundary，并作为索引 codec 的语义基准。
