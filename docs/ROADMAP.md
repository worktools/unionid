...
[#85](https://github.com/worktools/unionid/issues/85)、[#87](https://github.com/worktools/unionid/issues/87) 和 [#89](https://github.com/worktools/unionid/issues/89) 已完成 typed DML returning、稳定 mutation target 和批量 insert。当前 P1 核心切片 [#91](https://github.com/worktools/unionid/issues/91) 让 schema-aware prepare 覆盖 insert/upsert/update/delete，使重复应用写入在扫描前获得参数类型、完整 DML 校验与 schema 失效保护。它不依赖泛型或新执行模型。
