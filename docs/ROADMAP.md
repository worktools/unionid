...
[#85](https://github.com/worktools/unionid/issues/85) 已让 insert/upsert/update/delete 原子返回 typed 受影响行；[#87](https://github.com/worktools/unionid/issues/87) 已让 update/delete 在一次请求内稳定选择、修改并返回下一条任务。当前 P1 核心切片 [#89](https://github.com/worktools/unionid/issues/89) 增加 typed 批量 insert，让事件、任务初始化和同步 row list 统一经过默认值、约束、returning 与原子持久提交。它们都不依赖泛型或新执行模型。
