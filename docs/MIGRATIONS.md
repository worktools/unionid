...
- text 导入 UUID/bytes 必须显式使用纯函数 `uuid_parse old` 或 `bytes_parse_hex old`；没有隐式 cast。任一行解析失败返回 `E_SCALAR_LITERAL`，并回滚 schema、全部 rows 与 indexes。UUID 输出规范为小写 RFC 9562 文本，bytes parser 接受 hex 并保存原 octets。
