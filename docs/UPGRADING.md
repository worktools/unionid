...
v0.8 发布包同时携带 `RELEASE.json` 和独立的 `release/contract.json`。打包和验证脚本要求 Cargo、二进制 version report、manifest 与契约一致：

| 层 | v0.8 值 |
| --- | --- |
| unionid / 最低 Rust / redb | 0.8.0 / 1.94 / 4.1.0 |
...
v0.8.0 可直接打开 v0.7.0 的 format-6/7 数据库，不要求 storage upgrade、应用 schema migration 或源码重写。新增查询能力是向后兼容的：`count_distinct` 使用完整 typed equality；`avg int` 返回 `Option<float>`，float/duration avg 保留命名类型；`window` 要求显式 sort，并只追加 `int` 排名字段。静态查询生成物绑定软件版本，使用这些新 stage 或函数时应以 v0.8 `query rust` 重新生成并重新编译。
...
v0.8.0 可直接打开 v0.3.0–v0.5.0 的 format-6 数据库，也继续读取由 v0.5.0 显式启用增量备份后产生的 format-7 数据库。先在副本上运行 `doctor`、`check` 和应用读写。先按上节处理 Rust 形状源码与 range 语义；若应用使用静态查询生成物，再使用 v0.8.0 binary 对当前 schema 与全部 `.unid`（或兼容 `.uid`）查询重新运行 `query rust`，提交新的 digest，并重新编译客户端。v0.8.0 不要求 storage upgrade 或 schema migration；`init` 和 `project check` 只影响新项目与源码检查。
...
```bash
cp app.redb app-v0.8-rehearsal.redb
unionid doctor --db app-v0.8-rehearsal.redb --format json
unionid upgrade --db app-v0.8-rehearsal.redb --target 4
unionid upgrade --db app-v0.8-rehearsal.redb --target 5
unionid upgrade --db app-v0.8-rehearsal.redb --target 6
unionid check --db app-v0.8-rehearsal.redb
```
...
逻辑 `restore` 只写入不存在的新 redb 路径，便于并行验证和回退。v0.8 不承诺未来二进制自动原地升级内部格式，也不把正常重开当作备份。
...
v0.8.0 directly opens v0.7.0 format-6/7 databases without a storage upgrade, application schema migration, or source rewrite. The new query surface is additive: count_distinct uses complete typed equality; integer avg returns `Option<float>` while float/duration averages preserve named types; and window requires an explicit sort and only appends `int` rankings. Static query output is software-version-bound, so regenerate with the v0.8 `query rust` command and rebuild clients that use the new functions or stage. See the [v0.8.0 release notes](RELEASE-v0.8.0.md) for the full checklist.
