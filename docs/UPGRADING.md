...
v0.7 发布包同时携带 `RELEASE.json` 和独立的 `release/contract.json`。打包和验证脚本要求 Cargo、二进制 version report、manifest 与契约一致：

| 层 | v0.7 值 |
| --- | --- |
| unionid / 最低 Rust / redb | 0.7.0 / 1.94 / 4.1.0 |
...
v0.7.0 可直接打开 v0.6.0 的 format-6/7 数据库，不要求 storage upgrade 或应用 schema migration。升级的 breaking boundary 在源码：规范类型、值、pattern、bool operator 和 range 改为 Rust 形状。parser 暂时接受 v0.6 拼写，`unionid fmt` 可生成新规范形式；但旧 `take 2..4` 是闭区间，v0.7 的同一文本是半开区间。先逐条决定 `..` 或 `..=`，再运行 formatter、`project check` 和业务验收。静态查询生成物绑定软件版本，必须用 v0.7 `query rust` 重新生成并重新编译。

完整 checklist 与能力边界见 [v0.7.0 发布说明](RELEASE-v0.7.0.md)。

### 从 v0.3.0–v0.5.0 升级

v0.7.0 可直接打开 v0.3.0–v0.5.0 的 format-6 数据库，也继续读取由 v0.5.0 显式启用增量备份后产生的 format-7 数据库。先在副本上运行 `doctor`、`check` 和应用读写。先按上节处理 Rust 形状源码与 range 语义；若应用使用静态查询生成物，再使用 v0.7.0 binary 对当前 schema 与全部 `.unid`（或兼容 `.uid`）查询重新运行 `query rust`，提交新的 digest，并重新编译客户端。v0.7.0 不要求 storage upgrade 或 schema migration；`init` 和 `project check` 只影响新项目与源码检查。
...
```bash
cp app.redb app-v0.7-rehearsal.redb
unionid doctor --db app-v0.7-rehearsal.redb --format json
unionid upgrade --db app-v0.7-rehearsal.redb --target 4
unionid upgrade --db app-v0.7-rehearsal.redb --target 5
unionid upgrade --db app-v0.7-rehearsal.redb --target 6
unionid check --db app-v0.7-rehearsal.redb
```
...
逻辑 `restore` 只写入不存在的新 redb 路径，便于并行验证和回退。v0.7 不承诺未来二进制自动原地升级内部格式，也不把正常重开当作备份。
...
v0.7.0 directly opens v0.6.0 format-6/7 databases without a storage upgrade or application schema migration. The breaking boundary is source code: canonical types, values, patterns, boolean operators, and ranges adopt the Rust-shaped form. The parser temporarily accepts v0.6 spellings and `unionid fmt` emits the new form, but legacy `take 2..4` was inclusive while v0.7 interprets the same text as half-open. Decide explicitly between `..` and `..=` before formatting, running `project check`, and repeating application acceptance. Regenerate version-bound static queries with the v0.7 `query rust` command and rebuild clients. See the [v0.7.0 release notes](RELEASE-v0.7.0.md) for the complete checklist.

### Upgrading from v0.3.0 through v0.5.0

v0.7.0 directly opens v0.3.0–v0.5.0 format-6 databases and continues to read format-7 databases created when v0.5.0 explicitly enabled incremental backup. Run doctor, check, and application reads/writes on a copy first. Apply the Rust-shaped source and range review above, then rerun `query rust` with v0.7.0 over the current schema and every `.unid` (or compatible `.uid`) query, commit the new digests, and recompile the client. v0.7.0 requires no storage upgrade or schema migration; `init` and `project check` affect only new projects and source validation.
