...
v0.3 发布包同时携带 `RELEASE.json` 和独立的 `release/contract.json`。打包和验证脚本要求 Cargo、二进制 version report、manifest 与契约一致：

| 层 | v0.3 值 |
| --- | --- |
| unionid / 最低 Rust / redb | 0.3.0 / 1.94 / 4.1.0 |
...
公开 v0.1.0 使用 storage/catalog/value/index/migration/backup/protocol version 1，并不包含 receipt、cursor identity、生产标量、复合索引或 generation envelope。v0.2.0 已引入上表中的 format 6 边界；v0.3.0 保持相同内部格式和协议版本，只增加 Rust 接入、关联读取与 migration 使用体验。上表描述 v0.3.0 二进制的当前写入格式和全部可读范围，不追溯改写旧版本的发布契约。
...
```bash
cp app.redb app-v0.3-rehearsal.redb
unionid doctor --db app-v0.3-rehearsal.redb --format json
unionid upgrade --db app-v0.3-rehearsal.redb --target 4
unionid upgrade --db app-v0.3-rehearsal.redb --target 5
unionid upgrade --db app-v0.3-rehearsal.redb --target 6
unionid check --db app-v0.3-rehearsal.redb
```
