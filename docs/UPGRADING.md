...
v0.6 发布包同时携带 `RELEASE.json` 和独立的 `release/contract.json`。打包和验证脚本要求 Cargo、二进制 version report、manifest 与契约一致：

| 层 | v0.6 值 |
| --- | --- |
| unionid / 最低 Rust / redb | 0.6.0 / 1.94 / 4.1.0 |
...
v0.6.0 可直接打开 v0.3.0–v0.5.0 的 format-6 数据库，也继续读取由 v0.5.0 显式启用增量备份后产生的 format-7 数据库。先在副本上运行 `doctor`、`check` 和应用读写。若应用使用静态查询生成物，应使用 v0.6.0 binary 对当前 schema 与全部 `.unid`（或兼容 `.uid`）查询重新运行 `query rust`，提交新的 digest，并重新编译客户端。v0.6.0 不要求 storage upgrade 或 schema migration；`init` 和 `project check` 只影响新项目与源码检查。
...
```bash
cp app.redb app-v0.6-rehearsal.redb
unionid doctor --db app-v0.6-rehearsal.redb --format json
unionid upgrade --db app-v0.6-rehearsal.redb --target 4
unionid upgrade --db app-v0.6-rehearsal.redb --target 5
unionid upgrade --db app-v0.6-rehearsal.redb --target 6
unionid check --db app-v0.6-rehearsal.redb
```
...
逻辑 `restore` 只写入不存在的新 redb 路径，便于并行验证和回退。v0.6 不承诺未来二进制自动原地升级内部格式，也不把正常重开当作备份。
...
v0.6.0 directly opens v0.3.0–v0.5.0 format-6 databases and continues to read format-7 databases created when v0.5.0 explicitly enabled incremental backup. Run doctor, check, and application reads/writes on a copy first. Applications using generated static queries must rerun `query rust` with v0.6.0 over the current schema and every `.unid` (or compatible `.uid`) query, commit the new digests, and recompile the client. v0.6.0 requires no storage upgrade or schema migration; `init` and `project check` affect only new projects and source validation.
