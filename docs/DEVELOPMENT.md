...
- `tools/churn-eval` 从空 format-6 数据库运行固定 seed 的窄行、宽行和有限递归 ADT 长周期 workload。每轮组合 update/upsert/delete/batch、幂等重放与有界 receipt prune，前三轮还交错执行连续 shadow-generation migration；随后重开、完整 check、逻辑 backup，并逐项对照独立参考模型、ledger 和已清理的 maintenance state。JSON 保存逐轮 logical/file bytes、增长、receipt、migration phase/generation、维护时间、peak RSS 与 schema identity。CI 只跑小型结构 smoke；完整 10k/100k 证据由 #209 跟踪。
