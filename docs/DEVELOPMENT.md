...
当前全部 112 项测试通过：lib 单元测试 4 项、`tests/language.rs` 60 项、`tests/storage.rs` 25 项、`tests/migration.rs` 4 项、`tests/interfaces.rs` 11 项、`tests/codec.rs` 8 项，分别验证 Engine 注入提交失败、稳定 RowId 与过渡 snapshot 升级、语言/类型/查询及原子 update/delete/upsert、WAL 与 redb 的失败原子性/恢复、migration 历史约束、真实 CLI/TCP/并发请求，以及 ADT 值的稳定编码与损坏拒绝。TCP 测试实际启动服务，自动分配端口，并在结束时停止进程；所有持久化测试只使用隔离临时数据库。
...
- #13/#14/#15/#16：redb 固定内部表、版本化 codec、单写事务、稳定 RowId、原子 update/delete/upsert、明确／不确定提交错误、进程退出恢复矩阵和 `check --db` 已接入；继续补真实空间不足／同步故障、恢复时间边界、增量持久写、索引计划及 explain。
