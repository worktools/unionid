...
- v0.2 候选包在 `RELEASE.json` 记录 clean source commit；正式 verifier 拒绝显式允许生成的 dirty 开发包。macOS/Linux CI 在完整 suite、接口示例、评测 smoke 与包验证之后分别生成 machine-readable acceptance JSON，核对 GitHub head SHA、target、archive/manifest/contract digest，并保存逐类旅程证据而不复制业务数据。
