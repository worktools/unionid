...
- #181 已建立第一层执行接缝：`TypedRowSource` 统一 table stats、RowId lookup、row batch 与 typed index-hit batch，query 和 mutation target 不再直接遍历 resident row/posting；`CandidateRowSource` 只包装一次请求内已经合并的 persistent candidate root。source batch 固定为最多 1,024 rows／16 MiB，一般 query/mutation working state 固定为最多 250,000 rows／64 MiB，并记录不含业务值的 source observation。memory adapter 仍读取当前 persistent maps；redb MVCC view、snapshot cache 和 bounded open 属于 #182。
