...
- v0.9.0 增加 `unionid-query` 的 schema-aware 内联 Rust 查询宏，复用同一 parser/binder/codegen，并记录独立 consumer 的冷构建、增量重建、诊断和 migrated catalog 边界；storage、codec、backup 与 protocol 不变。
...
- redb open 与 durable commit 现在还发布固定、无业务值的阶段 profile。10k/100k 正式样本显示，100k open 的主要成本是完整派生索引重算/逻辑验证（p50 约 4.72 s）；100k migration durable commit 的主要成本是完整候选状态编码（约 41.01 s）和重载/验证前态（约 5.44 s），transaction apply+sync 仅约 0.76 s。原始 profile 与边界见 [M7 存储阶段记录](benchmarks/storage-phases-2026-09-09.md)。
- [RFC 0010](rfc/0010-bounded-resident-state-and-maintenance-generations.md) 据此选择共享 typed row source、绑定 redb MVCC transaction 的 committed view、snapshot-local bounded row cache，以及 format-6 Legacy0/shadow-generation migration。实现拆为 #181–#186，先证明 memory/source 语义一致，再让 format-5 indexed/page read 摆脱完整 resident rows，最后接入有界 full check/backup 与可恢复 cutover。
- #181 已建立第一层执行接缝：`TypedRowSource` 统一 table stats、RowId lookup、row batch 与 typed index-hit batch，query 和 mutation target 不再直接遍历 resident row/posting；`CandidateRowSource` 只包装一次请求内已经合并的 persistent candidate root。source batch 固定为最多 1,024 rows／16 MiB，一般 query/mutation working state 固定为最多 250,000 rows／64 MiB，并记录不含业务值的 source observation。
- #182 已把 format-5 Legacy0 接到该 seam：普通 open 发布绑定单一 redb MVCC root、schema/sequence/receipt 的 committed view，不遍历完整 rows/indexes；primary/equality/range/order/page 通过 durable index cursor 取得 RowId，再按需使用现有 ADT codec 解码。每个 committed view 拥有严格 32 MiB row cache，提交后的新 view 不复用旧 sequence 的 cache；反向遍历在 key 间反向、同 key RowId 内保持 memory 顺序。提交成功但新 read view 创建失败时返回 `E_STORAGE_REOPEN_REQUIRED` 并阻止当前 Engine 的旧状态读取。10k/100k 证据见 [Legacy0 有界读取记录](benchmarks/bounded-legacy-read-2026-09-09.md)。
- #183 已把 full scan 的 row-local stage、aggregate/group、blocking sort、NDJSON sink、single-statement redb mutation candidate、full check 与 logical backup 接到有界 source。filter/match/derive/select/take 逐行融合；aggregate 只保留 group/cell，sort 和 mutation change set 同时受 250,000 rows／64 MiB 限制。stream 不再先构造完整 `QueryResponse.rows`；check 用 row→index 和 index→row 点查代替完整派生 index set；backup format 4 通过两次有界读取保持原 checksum。100k check 从前一切片约 5.16 s／554 MiB 降到 1.44 s／80.62 MiB，原始证据见 [有界 full pipeline 记录](benchmarks/bounded-full-pipeline-2026-09-09.md)。旧 storage format 或非规范 catalog 的兼容写入仍使用完整候选，避免把 metadata-only candidate 送入 full-rebuild commit。
- #184 增加 storage format 6 的 generation catalog/row/index 和 maintenance manifest 固定表。新库从 Generated(1) 开始；format 5 可显式执行不扫描数据的 5→6 元数据升级并继续通过 Legacy0 有界读写。active/next generation、generation/manifest codec 和矛盾状态在打开时 fail closed；完整 check、增量提交、cursor、receipt、ledger 与 logical backup 都绑定同一 active generation。catalog/value/index component/migration/receipt/query/protocol codec 不变，旧二进制拒绝 format 6，回滚依赖升级前 logical backup。
- #185 在 format 6 上实现可恢复 shadow migration：source 读取固定为 1,024 rows／16 MiB，target transaction 不超过 32 MiB，manifest 持久化 checkpoint、计数、rolling digest、schema/file/executor identity。完整目标通过有界校验后原子切换 active generation 与 ledger，旧读快照继续绑定原 MVCC root，再以最多 1,024 entries 的事务回收。unfinished 状态允许查询并阻止普通写入；相同 apply 恢复，冲突文件 fail closed，`migration abort` 显式清理。
- #186 为 format-6 shadow migration 增加 value-free phase profile，并让 workload 保存查询 decode/cache/working bytes 与 migration generation/row/index/logical-byte 原始样本。100k open p95 约 12 ms，full check 约 1.54 s／96.33 MiB，shadow migration p95 约 16.51 s／427.98 MiB；10k 仍是舒适范围，100k 仍是已测上限。完整方法与接口矩阵见 [M7 验收记录](benchmarks/m7-acceptance-2026-09-10.md)。
- `unionid::scalars` 提供六种生产标量的 Rust 值域、规范 serde payload 与边界校验；原生 ScalarType/Value、无损 serde、protocol v2、完整 v1 typed-boundary 预检、按 boundary 选择的 `u1`/`u2` cursor、storage format 4 标量兼容层、format 5/6 durable codec、显式 upgrader、源码声明与精确运算均已接通。#137–#140 的验收由 PR #147–#151 完成，已实现范围见 [SCALARS.md](SCALARS.md)。
...
- v0.4.0 使用源码控制的 `release/contract.json` 冻结软件、最低 Rust、redb、version-report schema、storage/backup/codec、protocol 与 stream 版本。打包会核对 Cargo、二进制报告和 contract，将 contract 与版本对应 release notes 收入原生包；验证器再使用包外可信 contract 和 SHA-256 检查包内 contract、`RELEASE.json`、二进制与空目录教程。
- v0.4 候选包在 `RELEASE.json` 记录 clean source commit；正式 verifier 拒绝显式允许生成的 dirty 开发包。只在候选／tag Release workflow 中运行一次 macOS/Linux 完整 suite、静态查询与 typed application 验收、独立 consumer、接口示例、SQLite + SQLx 配对评测 smoke 与包验证，并分别生成 machine-readable acceptance JSON；普通 PR 与 main 仍只运行 Ubuntu 快速检查。
- v0.5.0 不再扩展数据库表达力；它把已交付的可观测性、compact fast no-op、增量备份/sequence restore 生命周期和 Envoy mTLS 路径收口到现有任务类 typed application 主链。候选版本统一冻结 Cargo、format-7 可读范围、journal codec 与发布说明，再只运行一次双平台 release workflow 和 Linux 大型备份评估。
...
- `tools/churn-eval` 从空 format-6 数据库运行固定 seed 的窄行、宽行和有限递归 ADT 长周期 workload。每轮组合 update/upsert/delete/batch、幂等重放与有界 receipt prune，前三轮还交错执行连续 shadow-generation migration；第一轮通过独立子进程按已提交 maintenance step 在 Building checkpoint、cutover 和单批 reclaim 后确定性退出并重开，最后用普通 apply 完成 cleanup，验证 ledger 不重复。随后完整 check、逻辑 backup，并以最多 10,000 行的主键范围块逐项对照独立参考模型、ledger 和已清理的 maintenance state。JSON 保存逐轮 logical/file bytes、增长、receipt、migration phase/generation、有界 checkpoint、维护时间、peak RSS 与 schema identity。普通 PR 只静态检查工具；1100-row 双平台结构 smoke 在 release workflow 运行。固定 6 轮的 10k/100k 证据显示 reclaim 后文件不会继续逐轮增长，但 redb 保留第一次 shadow migration 形成的约 46/745 MiB 高水位；完整方法、原始数据和运维解释见 [M9 churn 记录](benchmarks/churn-2026-09-10.md)。
- `tools/incremental-backup-eval` 在同一 ADT 工作负载上分别采集无 journal 和 format-7 journal 单行写入的完整原始样本、p50/p95、journal 时间与编码 bytes，并比较压缩 baseline/segment 和 logical backup 大小。restore-to-sequence 在独立子进程测量恢复、完整 check、逐页逐行等价检查与 peak RSS。普通 PR 只静态检查；release workflow 始终运行 100-row 双平台 smoke，10k/100k 仅通过显式 `run_large_backup_evaluator` 开关运行并保存 JSON artifact。方法见 [增量备份评估说明](benchmarks/incremental-backup.md)。
...
- 未分组 `aggregate` 与 `group ... aggregate` 支持 count/count_distinct/avg/decimal_avg/sum/min/max；count_distinct 复用完整 typed equality 并把去重键计入状态内存预算。avg 对 int 返回 float，对 float/duration 保留命名类型；decimal_avg 使用任意精度 coefficient 累计及显式目标 P/S、舍入模式；空输入返回 option None。sum 保留命名数值类型，min/max 返回 option。group key 使用完整 typed equality，可包含 ADT；后续 filter/select/sort/take 使用汇总后的 schema。group 数量、accumulator cell 与估算状态内存都有显式上限。
- 基础 `window` 以可选的显式 partition 和必需的 typed sort 计算 row_number/rank/dense_rank；省略 partition 时全部输入属于一个全局分区。stage 保留输入行数与行序并追加 int 字段。partition equality 支持完整 ADT，排名 state 与 materialized rows 共用 64 MiB working budget；page 组合、frame、lag/lead 和窗口 aggregate 保持延后。
