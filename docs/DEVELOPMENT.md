...
- #25/#81/#83/#85：已实现有限直接自递归 named ADT、穷尽 `match` update assignment 和 DML typed returning；互递归、用户泛型和递归查询函数继续根据实际场景分别设计。
