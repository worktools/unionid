...
- `tools/churn-eval` 从空 format-6 数据库运行固定 seed 的窄行、宽行和有限递归 ADT 长周期 workload。每轮组合 update/upsert/delete/batch、幂等重放与有界 receipt prune，前三轮还交错执行连续 shadow-generation migration；第一轮通过独立子进程按已提交 maintenance step 在 Building checkpoint、cutover 和单批 reclaim 后确定性退出并重开，最后用普通 apply 完成 cleanup，验证 ledger 不重复。随后完整 check、逻辑 backup，并逐项对照独立参考模型、ledger 和已清理的 maintenance state。JSON 保存逐轮 logical/file bytes、增长、receipt、migration phase/generation、有界 checkpoint、维护时间、peak RSS 与 schema identity。普通 PR 只静态检查工具；1100-row 双平台结构 smoke 在 release workflow 运行，完整 10k/100k 证据由 #209 跟踪。
