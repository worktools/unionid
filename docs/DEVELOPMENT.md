...
当前全部 94 项测试通过：`tests/language.rs` 54 项、`tests/storage.rs` 19 项、`tests/migration.rs` 4 项、`tests/interfaces.rs` 9 项、`tests/codec.rs` 8 项，分别验证语言/类型/查询、WAL 与 redb 的失败原子性/恢复、migration 历史约束、真实 CLI/TCP/并发请求，以及 ADT 值的稳定编码与损坏拒绝。TCP 测试实际启动服务，自动分配端口，并在结束时停止进程；所有持久化测试只使用隔离临时数据库。
...
- #13/#14/#15/#16：redb 固定内部表、版本化 codec 和单写事务已接管新的 `--db` 入口；继续补故障注入与恢复状态、CRUD/upsert、增量持久写、索引计划及 explain。
