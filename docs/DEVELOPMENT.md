...
- 基础 `window` 以显式 partition 和 typed sort 计算 row_number/rank/dense_rank，保留输入行数与行序并追加 int 字段。partition equality 支持完整 ADT，排名 state 与 materialized rows 共用 64 MiB working budget；page 组合、frame、lag/lead 和窗口 aggregate 保持延后。
