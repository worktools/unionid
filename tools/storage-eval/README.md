...
5. 后端本身是否阻止同一路径被第二个写实例打开；SQLite 允许多连接，因此 unionid 仍需保留自己的所有权锁。
...
SQLite 探针使用 WAL、`synchronous=FULL` 和 `fullfsync=ON`；后者在不支持的平台无效果。探针模拟应用进程在提交前或提交返回后立即退出，不模拟断电、磁盘缓存丢失或文件系统损坏。选型结论与完整限制见 [ADR 0001](../../docs/adr/0001-sqlite-storage.md)。
