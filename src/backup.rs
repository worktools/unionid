...
pub fn create(db: impl Into<PathBuf>, output: impl AsRef<Path>) -> Result<BackupInfo> {
    ...
    write_database(engine.database_snapshot(), output.as_ref())
}

pub fn restore(backup: impl AsRef<Path>, db: impl Into<PathBuf>) -> Result<BackupInfo> {
    let (database, info) = read_database(backup.as_ref())?;
    drop(Engine::restore_redb(db.into(), database)?);
    Ok(info)
}
...
pub fn import_legacy(
    ...
    let info = info(&database)?;
    drop(Engine::restore_redb(db.into(), database)?);
    Ok(info)
}

fn write_database(database: Database, output: &Path) -> Result<BackupInfo> {
...
fn read_database(path: &Path) -> Result<(Database, BackupInfo)> {
    let file =
...
fn info(database: &Database) -> Result<BackupInfo> {
