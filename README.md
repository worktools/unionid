...
Remote calls can use `unionid::client::TcpClient` (`request`/`request_retrying` plus `typed_rows`); async frameworks can enable the optional `asynchronous` feature and run the same entry point on a blocking worker via `unionid::asynchronous::execute_protocol_request`. With the `http` feature, `unionid::asynchronous::http::router` supplies versioned query, NDJSON stream, and cancellation routes without application-owned worker or stream bridging code. Integration types such as `ConcurrentEngine` are re-exported at the crate root.
