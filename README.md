...
关联读可用 `Engine::fetch_by_key` / `typed_fetch_by_key` 按主键或唯一索引批量取回，结果与输入键等长同序、缺失键为 `None`，并基于一致快照；非索引键或非唯一命中分别返回 `E_RELATION_KEY` / `E_RELATION_NOT_UNIQUE`。设计见 [RFC 0013](docs/rfc/0013-minimal-relational-reads.md)。
